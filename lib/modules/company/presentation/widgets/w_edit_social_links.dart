import 'package:flutter/material.dart';
import 'package:flutter_modular/flutter_modular.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:monster/core/extension/context_extension.dart';
import 'package:monster/core/services/forms/f_edit_social_links.dart';
import 'package:monster/core/widgets/w_app_button.dart';
import 'package:monster/modules/company/presentation/cubit/company_cubit.dart';
import 'package:monster/modules/job_seeker/presentation/widgets/w_remove_icon.dart';

class WEditSocialLinks extends StatefulWidget {
  const WEditSocialLinks({super.key});

  @override
  State<WEditSocialLinks> createState() => _WEditSocialLinksState();
}

class _WEditSocialLinksState extends State<WEditSocialLinks> {
  late CompanyCubit companyCubit;

  @override
  void initState() {
    super.initState();
    companyCubit = Modular.get<CompanyCubit>();
    companyCubit.fillReferencesForms();
    companyCubit.fEditSocialLinks.init();
    companyCubit.socialLinkForms.add(companyCubit.fEditSocialLinks);
  }

  @override
  Widget build(BuildContext context) {
    return Container(
      constraints: BoxConstraints(
        maxHeight: companyCubit.socialLinkForms.length > 1
            ? MediaQuery.of(context).viewInsets.bottom < 30
                ? 600.h
                : 330.h
            : 250.h,
        minHeight: 10,
      ),
      child: ListView(
        padding: EdgeInsets.symmetric(
          horizontal: 2.w,
          vertical: 4.h,
        ),
        children: [
          ...companyCubit.socialLinkForms.map(
            (reference) {
              return Column(
                spacing: 16.h,
                children: [
                  Row(
                    children: [
                      Expanded(
                        flex: 1,
                        child: reference.nameField.buildField(context),
                      ),
                      12.horizontalSpace,
                      InkWell(
                        onTap: () {
                          setState(() {
                            companyCubit.socialLinkForms.remove(reference);
                          });
                        },
                        child: const WRemoveIcon(),
                      ),
                    ],
                  ),
                  reference.linkField.buildField(context),
                  28.verticalSpace,
                ],
              );
            },
          ),
          WAppButton(
            title: 'Add New',
            isFilled: false,
            width: context.width,
            onTap: () {
              setState(() {
                companyCubit.socialLinkForms.add(FEditSocialLinks()..init());
              });
            },
          ),
        ],
      ),
    );
  }
}
